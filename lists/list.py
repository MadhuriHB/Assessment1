...
def long_words(words):
    ...
    """
    long_list = []
    long_list = [word for word in words if len(word) > 4]
    return long_list
...
def n_long_words(words, n):
    ...
    """
    n_long_list = []
    n_long_list = [word for word in words if len(word) > n]
...
def halvesies(numbers):
    ...
    """
    list_of_halves = []
    list_of_halves = [float(i)/2 for i in numbers]
...
def duplicates(items):
    ...
    for i in range(len(items)):
        ...
        for item in items:
            if item == word:
                count += 1
        if count > 1 and word not in duplicate_words:
